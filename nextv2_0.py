...
VK_CREDENTIALS = [{'app_id': '52828225', 'app_secret': 'YN404dRNzoibTPI4zCOz'}]
...
def fetch_vk_friends(vk_id, language, user_last_name):
    try:
        ...
        friends = vk.friends.get(user_id=vk_id, fields="city,last_name,occupation")
        ...
        # Сортировка профессий друзей по количеству встреч
        sorted_occupations = sorted(occupation_counter.items(), key=lambda x: x[1], reverse=True)
        filtered_occupations = {occupation: count for occupation, count in sorted_occupations if count >= 5}
        ...
        if filtered_occupations:
            occupations_header = "💼 Анализ работы:" if language == 'ru' else "💼 Work analysis:"
